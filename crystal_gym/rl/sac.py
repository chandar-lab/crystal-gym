"""
Soft Actor-Critic (SAC) implementation for crystal design optimization.

SAC is an off-policy algorithm that maximizes both expected return and entropy,
encouraging exploration while maintaining stability.
"""
...
class SoftQNetwork(nn.Module):
    ...
    Estimates Q-values for state-action pairs. Supports both MLP and MEGNetRL architectures.
    Note: Sharing encoders between Actor and Critics is not recommended for SAC without
    stopping actor gradients (see SAC+AE paper https://arxiv.org/abs/1910.01741).
    """
    ...
    def __init__(self, envs: gym.Env, network_type: str = "mlp") -> None:
        ...
        if network_type == "mlp":
            # MLP architecture (originally designed for image observations)
...
class Actor(nn.Module):
    ...
    Learns a stochastic policy that maximizes both expected return and entropy.
    Supports both MLP and MEGNetRL architectures.
    """
    ...
    def get_action(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple of (action, log_prob, action_probs)
        """
        if self.network_type == "mlp":
            logits = self(x / 255.0)
        elif self.network_type == "MEGNetRL":
...
def main(args: DictConfig) -> None:
    """
    Main training function for SAC agent.
    ...
    # Initialize networks
    actor = Actor(envs, network_type="MEGNetRL").to(device)
    qf1 = SoftQNetwork(envs, network_type="MEGNetRL").to(device)
    qf2 = SoftQNetwork(envs, network_type="MEGNetRL").to(device)
    qf1_target = SoftQNetwork(envs, network_type="MEGNetRL").to(device)
    qf2_target = SoftQNetwork(envs, network_type="MEGNetRL").to(device)
    qf1_target.load_state_dict(qf1.state_dict())
    qf2_target.load_state_dict(qf2.state_dict())
    
    # Initialize optimizers (eps=1e-4 increases numerical stability)
    ...
    for global_step in range(start_iteration, args.algo.total_timesteps):
        ...
        if global_step < args.algo.learning_starts:
            # Random actions during initial exploration phase
        ...
        if global_step > args.algo.learning_starts:
            if global_step % args.algo.update_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    next_q_value = (rewards_sampled.flatten() + 
                                  (1.0 - dones_sampled.flatten().to(torch.float32)) * 
                                  args.algo.gamma * min_qf_next_target)
